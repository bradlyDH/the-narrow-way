...
//       setOutgoing(out || []);
//     } catch (e) {
//       console.log('[make-friends] loadLists error raw:', e);
//       setIncoming([]);
//       setOutgoing([]);
...
//   // 👇 ADD THIS: realtime subscription for incoming/outgoing lists
//   useEffect(() => {
//     let channel;
//     (async () => {
//       const {
...
//       console.log('[make-friends-rt] using filters:', reqFilter, addFilter);
//       resetRealtime();
...
//   // Replace your current sendRequest with this:
//   const sendRequest = async () => {
//     if (!target) return;
//     setSending(true);
//     try {
//       // uses the service → handles cleanup + insert + sync
...
//       await loadLists(); // optional refresh
//     } catch (e) {
...
const L = createTaggedLogger('MakeFriends');
...
export default function MakeFriendsScreen({ navigation }) {
  ...
  const loadLists = useCallback(async () => {
    ...
    try {
      ...
      setOutgoing(out || []);
      L.debug(
        'loadLists ok incoming=',
        (inc || []).length,
        'outgoing=',
        (out || []).length
      );
    } catch (e) {
      L.error('loadLists error:', e?.message || e);
      setIncoming([]);
      setOutgoing([]);
      Alert.alert('Error', e?.message || String(e));
    } finally {
  ...
  // realtime subscription for incoming/outgoing lists
  useEffect(() => {
    let channel;
    (async () => {
      const {
