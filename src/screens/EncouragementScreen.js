...
// //   // If we navigated from Friends, prefill
// //   useEffect(() => {
...
// //     if (idFromRoute) setRecipientId(idFromRoute);
// //   }, [route.params]);
...
// //       // fetch my display name (for from_display when sending)
...
// //       // most recent received → fallback to most recent sent
// //       const { data: received, error: recErr } = await supabase
// //         .from('encouragements')
// //         .select('id, message_text, created_at, from_display')
...
// //       // Insert using the app-driven fields (no template table needed)
// //       const { error } = await supabase.from('encouragements').insert({
// //         sender_id,
// //         recipient_id: recipientId || null, // keep null if sending to a typed name only
// //         to_name: to,
// //         template_key: selected.key,
// //         message_text: selected.text,
// //         from_display,
// //         // created_at is default now() from migration
// //       });
...
//   // NEW: keep the selected friend id (for recipient_id)
//   const [recipientId, setRecipientId] = useState(null);

//   // NEW: friends (accepted) for type-ahead
...
//     const q = (toName || '').trim().toLowerCase();
//     if (!q) return [];
//     // match display_name contains OR short_id startsWith/contains
//     return friends
//       .filter((p) => {
//         const name = (p.display_name || '').toLowerCase();
//         const sid = (p.short_id || '').toLowerCase();
...
//     // prefer display name; fall back to short_id so the user sees *something*
...
//       const { error } = await supabase.from('encouragements').insert({
//         sender_id,
//         recipient_id: recipientId || null, // use chosen friend id when available
//         to_name: to,
//         template_key: selected.key,
//         message_text: selected.text,
//         from_display,
...
//           onChangeText={(t) => {
//             setToName(t);
//             // if the user starts editing, clear any previously selected recipient
//             if (recipientId) setRecipientId(null);
//           }}
//           maxLength={60}
//         />
